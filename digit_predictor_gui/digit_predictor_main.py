...
class DrawingCanvas(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # 将 QImage 填充为白色。
        # 在 PyQt6 中，颜色常量不再直接挂在 Qt 下，而是通过 Qt.GlobalColor 枚举访问。
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PyQt 数字预测")  # 设置主窗口标题
        ...
        layout.addWidget(self.canvas)  # 将画布添加到主布局中

        self.canvas.set_pen_size(5) # 设置初始画笔粗细 为5
        ...
        clear_button = QPushButton("预测")  # 清空画布按钮
        clear_button.clicked.connect(self.canvas.clear_canvas)  # 连接按钮的点击信号到清空画布方法
        button_layout.addWidget(clear_button)
...
if __name__ == '__main__':
    ...
    window.show()  # 显示主窗口
    sys.exit(app.exec())  # 启动应用程序的事件循环。在 PyQt6 中，app.exec_() 改名为 app.exec()。